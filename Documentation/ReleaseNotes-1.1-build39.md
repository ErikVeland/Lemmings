...
Build: 39
Release commit: 589835301729b5fd0f5e0370d3e0f2a95e34cc0e
...
game hid the system cursor during pointer capture but did not draw the
gameplay reticle when no skill was selected, so the cursor disappeared. In
Lemmings 2, a dead reference to a removed `canvas.pointers` property caused
the same class of failure.

This candidate also fixes a click-targeting bug in Classic. A follower lemming
behind a clicked bridge builder did not receive the selected skill when it
stood outside the click's own pick box. The game now searches for that
follower near the builder instead of near the click.

This candidate carries the 1.2 content browser feature, already on this
branch before the candidate.
...
- 5898353 Fix builder-follower click targeting missing outside pick box
- 777a91a L2 Cursor
- a03ef13 L3 Cursor
- 40500e9 Cursory fixes
- dd9c1a1 Make the selected skill a tiny cursor companion
- e3ccb4a Keep cursor reticle clear over gameplay controls
- 6f8d749 Clamp skill cursor badge within drawing bounds
- 99fa61e Merge remote-tracking branch 'origin/codex/1-1-qol-target-select' into codex/release-1.2
- 9845e41 Cut 1.2 content browser and cursor hotfix
...
A	Sources/LemmingsLocal/LevelCoverFlowView.swift
A	Sources/LemmingsLocal/LevelPlaylistStore.swift
A	Sources/LemmingsLocal/LevelPreviewStore.swift
A	Sources/LemmingsLocal/MusicFileDeck.swift
M	Sources/LemmingsLocal/MusicPlayer.swift
M	Sources/LemmingsLocal/PanelGlyphs.swift
...
M	Sources/LemmingsLocal/SequelArtworkRenderer.swift
...
M	Sources/NxlvKit/ClassicGameFlow.swift
M	Sources/NxlvKit/ClassicSaga.swift
...
A	Sources/NxlvKit/LevelCatalogue.swift
A	Sources/NxlvKit/LevelPlaylist.swift
