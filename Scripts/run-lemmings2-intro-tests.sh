...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/l2-intro-tests"
...
swiftc -swift-version 6 -warnings-as-errors \
  ...
  -o "$build_dir/IntroTests" "$project_dir/Tests/Lemmings2IntroTests/main.swift"
if [[ -f "$project_dir/Sources/Ports/Lemm2/LEVELS/LEVEL000.DAT" ]]; then
  "$build_dir/IntroTests" "$project_dir/Sources/Ports/Lemm2"
else
  "$build_dir/IntroTests"
fi
