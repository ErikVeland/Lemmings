...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/save-recovery-tests"
...
swiftc -O -swift-version 6 -warnings-as-errors -target "$(uname -m)-apple-macos13.0" \
  ...
  Sources/LemmingsLocal/ArcadeRecordFile.swift Sources/LemmingsLocal/LegacySaveMigration.swift \
  Tests/SaveRecoveryTests/main.swift
"$build_dir/tests"
