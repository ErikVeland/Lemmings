...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/mac-ui-tests"
...
swiftc -swift-version 6 -warnings-as-errors \
  ...
  -o "$build_dir/MacUITests" "$project_dir/Tests/ClassicMacUITests/main.swift"
"$build_dir/MacUITests" "${1:-$project_dir/.build/mac-artwork/export}"
