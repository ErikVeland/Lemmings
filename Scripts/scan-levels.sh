...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/scan-levels"
mkdir -p "$build_dir/modules"
swiftc -swift-version 6 -parse-as-library -O -emit-module -emit-library \
  -module-name NxlvKit \
...
swiftc -swift-version 6 -O -I "$build_dir/modules" -L "$build_dir" -lNxlvKit \
  ...
  -o "$build_dir/ScanLevels" "$project_dir/Tools/ScanLevels/main.swift"
"$build_dir/ScanLevels" "$@"
