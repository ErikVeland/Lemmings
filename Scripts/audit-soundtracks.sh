...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/soundtrack-audit"
...
swiftc -swift-version 6 -I "$build_dir/modules" -L "$build_dir" -lNxlvKit \
  ...
  -o "$build_dir/SoundtrackAudit" "$project_dir/Tools/SoundtrackAudit/main.swift"
cd "$project_dir" && "$build_dir/SoundtrackAudit"
