...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/game-flow-tests"
...
swiftc -swift-version 6 -warnings-as-errors \
  ...
  -o "$build_dir/GameFlowTests" "$project_dir/Tests/ClassicGameFlowTests/main.swift"
"$build_dir/GameFlowTests" "${1:-$project_dir/Content/lemming1.pc}"
