...
set -euo pipefail
project_dir="${0:A:h:h}"
build_dir="$project_dir/.build/mac-level-tests"
image="${1:-$project_dir/Sources/Ports/lemmings_1_5_2/Lemmings_1_5_2.dsk}"
if [[ ! -f "$image" ]]; then
  print "No Macintosh disk image at $image"
  exit 0
fi
...
swiftc -swift-version 6 -warnings-as-errors \
  ...
  -o "$build_dir/MacLevelTests" "$project_dir/Tests/ClassicMacLevelTests/main.swift"
"$build_dir/MacLevelTests" "$image"
